...
static void ShowExampleAppCustomNodeGraph(bool* opened)
{
    ...
    struct Node
    {
        ...
        bool foobar(int slot_no) {
            return false;//ImGui::ItemHoverable(, slot_no);
        }
    };
    struct NodeLink
    {
    ...
    int node_slot_hovered_in_scene = -1;
    ...
    for (int node_idx = 0; node_idx < nodes.Size; node_idx++)
    {
        ...
        draw_list->ChannelsSetCurrent(0); // Background
        ImGui::SetCursorScreenPos(node_rect_min);
        ImGui::InvisibleButton("node_slot", ImVec2(NODE_SLOT_RADIUS, NODE_SLOT_RADIUS));
        if (ImGui::IsItemHovered())
        {
            node_slot_hovered_in_scene = node->ID;
            open_context_menu |= ImGui::IsMouseClicked(1);
        }
        ...
        ImU32 node_input_slot_color = (node_hovered_in_list == node->ID || node_slot_hovered_in_scene == node->ID || (node_hovered_in_list == -1 && node_selected == node->ID)) ? IM_COL32(75, 75, 75, 255) : IM_COL32(60, 60, 60, 255);
        ...
        for (int slot_idx = 0; slot_idx < node->InputsCount; slot_idx++)
            draw_list->AddCircleFilled(offset + node->GetInputSlotPos(slot_idx), NODE_SLOT_RADIUS, node_input_slot_color);
        for (int slot_idx = 0; slot_idx < node->OutputsCount; slot_idx++)
            draw_list->AddCircleFilled(offset + node->GetOutputSlotPos(slot_idx), NODE_SLOT_RADIUS, node_input_slot_color);
