...
const std::string glsl_functions = R"(
uniform float time;
...
const std::string noies_functions = R"(
//
...
namespace Shader {
    ...
    public:
        ...
        std::shared_ptr<GlslShader> get_shader() const;
        std::vector<std::shared_ptr<Port>> get_uniforms() const;
    ...
    private:
        ...
        std::vector<std::shared_ptr<Port>> uniforms;

        Shader::GLSLCode shader_code;
        std::shared_ptr<GlslShader> shader;
        bool compile_code = false;
    };
}